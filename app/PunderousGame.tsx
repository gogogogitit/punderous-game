...
export default function Component() {
  const [puns, setPuns] = useState<Pun[]>([
    ...
    { id: 3, question: "Why did the scarecrow win an award?", answer: "They were outstanding in their field", difficulty: 2, upVotes: 0, downVotes: 0 },
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        if (lastPlayedDate !== today) {
          currentPunIndex = (currentPunIndex + 1) % puns.length;
        ...
        setGameState(prev => {
          const newState = {
            ...
            usedPunIds: new Set([puns[currentPunIndex].id])
          };
      ...
      analyticsTrackEvent('game_started', {
        event_category: 'Game',
  ...
  const handleAnswerSubmit = useCallback(async () => {
    ...
    if (!allWordsValid) {
      ...
      analyticsTrackEvent('invalid_word_submitted', {
        event_category: 'Game',
    ...
    if (compareAnswers(userGuess, correctAnswer) || allLettersRevealed) {
      ...
      analyticsTrackEvent('correct_answer', {
        event_category: 'Game',
      ...
      if (newAttempts === 0) {
        ...
        analyticsTrackEvent('incorrect_answer', {
          event_category: 'Game',
  ...
  const handleSkip = useCallback(() => {
    ...
    analyticsTrackEvent('skipped_pun', {
      event_category: 'Game',
  ...
  const handleEmailSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      analyticsTrackEvent('email_submission_error', {
        event_category: 'User',
    ...
    try {
      ...
      if (result.success) {
        setPuns(prevPuns => prevPuns.map(p => 
         ...
         analyticsTrackEvent('vote_submitted', {
           event_category: 'Game',
  ...
  const handleDonation = useCallback((platform: 'paypal' | 'venmo') => {
    ...
    analyticsTrackEvent('donation_link_clicked', {
      event_category: 'User',
          ...
          onClick={() => {
            ...
            if (navigator.share) {
              navigator.share({
                ...
                analyticsTrackEvent('game_shared', {
                  event_category: 'Game',
                ...
                analyticsTrackEvent('share_error', {
                  event_category: 'Game',
